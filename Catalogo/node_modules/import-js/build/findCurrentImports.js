...
function convertToImportStatement(node, source) {
  if (node.type === 'ImportDeclaration') {
    ...
    return new _ImportStatement.default({
      ...
      danglingCommas: DANGLING_COMMAS.test(source)
    });
  }
...
function findCurrentImports(config, currentFileContent, ast) {
  var result = {
    imports: new _ImportStatements.default(config),
    range: {
  ...
  ast.program.body.forEach(function (node, index) {
    ...
    var importStatement = convertToImportStatement(node, currentFileContent.slice(node.start, node.end));
    ...
    if (!result.imports.empty() && !config.get('groupImports') && !config.get('sortImports') && index > 0) {
      var previousNodeEnd = ast.program.body[index - 1].end; // Advance past the end of the line
